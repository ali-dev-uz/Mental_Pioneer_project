...
async def daily_message_set_ar(msg: types.Message):
    ...
    await Personaldata.Daily7Messages_ar.manual_done.set()


@dp.callback_query_handler(state=Personaldata.Daily7Messages_ar.manual_done, text=['agree', 'back'])
async def manual_done_callback_ar(call: types.CallbackQuery, state: FSMContext):
    if call.data == 'back':
        ...
        await db.delete_cycle_message_ar(day_id=7)
        await Personaldata.Daily7Messages_ar.input_message.set()


@dp.message_handler(state=Personaldata.Daily7Messages_ar.input_message, content_types=types.ContentType.ANY)
async def input_message_handler_ar(messages: types.Message):
    ...
    await db.add_cycle_message_ar(day_id=7, message_id=send_message.message_id)



async def Sunday_Cycle_Channel_ar(weekly_format, name_weekly):
    if name_weekly == "Monday":
        all_content = await db.select_cycle_message_ar(day_id=2)
        for mass in all_content:
            await dp.bot.copy_message(chat_id=-1002028676437,
        ...
        for mass in all_content:
            channel_smg = await dp.bot.copy_message(chat_id=-1002028676437,
                                                    from_chat_id=-1001899486063,
                                                    message_id=int(mass['message_id']))
            await db.update_end_message_id(end_message_id=channel_smg.message_id,
                                           chats_id=-1002028676437)
            await asyncio.sleep(0.1)
    elif name_weekly == "Sunday":
        all_content = await db.select_cycle_message_ar(day_id=1)
        await channel_cleaning(-1002028676437)
        await remove_user_to_channel_and_send_test(chat_id=-1002028676437,
